...
@section('hero')
    <div class="hero-section">
        <div class="hero-content">
            <h1 class="hero-title">
                <i class="fas fa-tag"></i> Gestão de Marcas
            </h1>
            <p class="hero-subtitle">Gerencie todas as marcas cadastradas no sistema</p>
        </div>
    </div>
@endsection
        ...
        <div class="card-body">
                        ...
                        <tbody>
                            @foreach($brands as $brand)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge">{{ $brand->vehicles_count }}</span>
                                    </td>
                                    <td>{{ $brand->created_at->format('d/m/Y') }}</td>
                                    <td>
                                        <div class="action-buttons">
                                            <a href="{{ route('admin.brands.edit', $brand->id) }}" class="btn-warning">
                                                <i class="fas fa-edit"></i> Editar
                                            </a>
                                            <form action="{{ route('admin.brands.destroy', $brand->id) }}" method="POST" class="inline-form">
                                                @csrf
                                                @method('DELETE')
            ...
            @else
                <div class="empty-state">
                    ...
                    <p>Comece adicionando sua primeira marca ao sistema</p>
                    <a href="{{ route('admin.brands.create') }}" class="btn-primary">
                        <i class="fas fa-plus"></i> Adicionar Primeira Marca
                    </a>
                </div>
            @endif
        </div>
